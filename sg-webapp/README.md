...
Track your subscriptions and see how much time and money you actually spend on them.

**Two-part system:**
- **sg-webapp** (this repo) — Django backend + dashboard
...
uv run src/manage.py migrate     # Create tables
uv run src/manage.py createsuperuser  # Create admin user
uv run src/manage.py runserver   # Start dev server
```
...
uv run src/manage.py test api.tests.SessionBuilderTest  # Single test class
